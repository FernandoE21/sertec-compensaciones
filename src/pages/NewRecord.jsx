...
function NewRecord() {
  ...
  useEffect(() => {
    ...
    const cargarOnomastico = async () => {
      ...
      const { data: registrosOnom } = await supabase.from('nuevo_registro_horas').select('id, estado').eq('codigo_trabajador', codigo).eq('tipo_solicitud', 'ONOMÁSTICO').gte('fecha_hora_inicio', primerDiaMes.toISOString().split('T')[0]).lte('fecha_hora_inicio', ultimoDiaMes.toISOString().split('T')[0])
  ...
  const guardarRegistro = async (e) => {
    ...
    setEnviando(false)
    if (error) Swal.fire('Error', error.message, 'error')
    else { logBitacora({ usuario: codigo, tipo_usuario: 'empleado', accion: esEdicion ? 'editar' : 'crear', modulo: 'nuevo_registro_horas', descripcion: `${esEdicion ? 'Editó' : 'Creó'} solicitud ${tipoSolicitud}`, registro_id: esEdicion ? String(nroRegistro) : null, datos_nuevos: { tipo_solicitud: tipoSolicitud, fecha: fechaDia, inicio: realInicio, fin: realFin } }); Swal.fire({ title: '¡Procesado!', text: 'Registro guardado correctamente', icon: 'success', timer: 1500, showConfirmButton: false }); navigate(`/registros/${codigo}`) }
  }
