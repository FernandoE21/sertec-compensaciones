...
import { FileSpreadsheet, FilterX, Info, X } from 'lucide-react'
...
function AdminUserRecords() {
  ...
  const handleEstadoChange = async (id, nuevoEstado) => {
    ...
    if (!error) {
      setRegistros(registros.map(r => r.id === id ? { ...r, estado: nuevoEstado } : r))
      if (registroDetalle && registroDetalle.id === id) setRegistroDetalle({ ...registroDetalle, estado: nuevoEstado })
      const adminUsuario = sessionStorage.getItem('admin_usuario') || 'admin'
      logBitacora({ usuario: adminUsuario, tipo_usuario: 'admin', accion: nuevoEstado === 'Aprobado' ? 'aprobar' : nuevoEstado === 'Rechazado' ? 'rechazar' : 'editar', modulo: 'registro_horas', descripcion: `${nuevoEstado === 'Aprobado' ? 'Aprobó' : nuevoEstado === 'Rechazado' ? 'Rechazó' : 'Cambió estado de'} solicitud de ${registro?.nombre_empleado || codigo} (${registro?.tipo_solicitud || ''})`, registro_id: String(id), datos_anteriores: { estado: registro?.estado }, datos_nuevos: { estado: nuevoEstado } })
  ...
  const calcularHorasFila = (reg) => {
    ...
    const tiposFavor = ["COMPENSACIÓN POR TRASLADO DE VIAJE", "COMPENSACIÓN POR TRASLADO DE VIAJE - CONDUCTOR", "COMPENSACIÓN POR TRASLADO DE VIAJE - COPILOTO", "COMPENSACIÓN POR TRASLADO DE EQUIPOS", "SOBRETIEMPO EN CLIENTE", "SOBRETIEMPO EN SERTEC", "SOBRETIEMPO POR TRASLADO DE VIAJE"]
    const tiposContra = ["COMPENSACIÓN A FAVOR DE SERTEC", "POR SALIDAS ANTES DE HORARIO", "POR INGRESO FUERA DE HORARIO"]
    if (tiposFavor.includes(reg.tipo_solicitud)) return <span className="inline-block bg-green-50 text-green-700 px-2 py-0.5 rounded-full text-[11px] font-bold">+{texto}</span>
    if (tiposContra.includes(reg.tipo_solicitud)) return <span className="inline-block bg-red-50 text-red-700 px-2 py-0.5 rounded-full text-[11px] font-bold">-{texto}</span>
    return <span className="text-gray-500 text-[11px]">{texto}</span>
  }
  ...
  const statusColor = (e) => {
    if (e === 'Aprobado') return 'bg-green-50 text-green-700 border-green-200'
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead>
              <tr className="bg-corporate-blue text-white">
                ...
                <th className="py-3 px-3 text-center text-xs font-bold">Detalle</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {registros.map((reg) => (
                <tr key={reg.id} className={`hover:bg-gray-50/50 transition-colors ${reg.estado === 'Rechazado' ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="py-3 px-3 text-center">
                    <select
                      className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase border appearance-none cursor-pointer outline-none w-[100px] text-center ${statusSelectCls(reg.estado)}`}
                  ...
                  <td className="py-3 px-3 text-center">
                    <button onClick={() => setRegistroDetalle(reg)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-sky-50 text-sky-600 hover:bg-sky-100 transition-colors border-none cursor-pointer mx-auto" title="Ver detalle">
                      <Info size={15} />
                    </button>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setRegistroDetalle(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto animate-slide-in p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
              <h3 className="text-lg font-bold text-gray-800">Detalle (Admin)</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select className={`block mt-1 px-2.5 py-1.5 rounded-xl text-sm font-bold border outline-none ${statusSelectCls(registroDetalle.estado)}`} value={registroDetalle.estado} onChange={(e) => handleEstadoChange(registroDetalle.id, e.target.value)}>
                  <option value="Pendiente">Pendiente</option><option value="Aprobado">Aprobado</option><option value="Rechazado">Rechazado</option>
                </select>
              </div>
              <div className="col-span-2"><span className="text-[10px] font-bold uppercase text-gray-400">Solicitud</span><p className="text-sm font-medium text-gray-800">{registroDetalle.tipo_solicitud}</p></div>
              <div><span className="text-[10px] font-bold uppercase text-gray-400">Fecha</span><p className="text-sm text-gray-700">{new Date(registroDetalle.fecha_hora_inicio).toLocaleDateString()}</p></div>
              ...
              <div className="col-span-2"><span className="text-[10px] font-bold uppercase text-gray-400">Motivo</span><p className="text-sm bg-gray-50 p-2.5 rounded-lg border border-gray-200 text-gray-700">{registroDetalle.motivo || 'Sin detalles'}</p></div>
              <div className="col-span-2"><span className="text-[10px] font-bold uppercase text-gray-400">Creado</span><p className="text-xs text-gray-400">{new Date(registroDetalle.created_at).toLocaleString()}</p></div>
            </div>
            <div className="mt-5 flex justify-end border-t border-gray-200 pt-4">
              <button onClick={() => setRegistroDetalle(null)} className="flex items-center gap-1 px-4 py-2 text-sm font-semibold text-gray-500 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 cursor-pointer transition-colors">
                Cerrar
              </button>
            </div>
          </div>
